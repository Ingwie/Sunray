...
int16_t WiFiClass::begin(char* ssid)
{
  return 0;
}

int16_t WiFiClass::begin(char* ssid, uint8_t key_idx, const char *key)
{
  return 0;
}

int16_t WiFiClass::startWifiProtectedSetup()
{
  wxLogMessage("starting WiFi protected setup - NOTE: press WPS button on your router first...");
  //wxLogMessage(passphrase);
  wxArrayString output;
  if (!wxExecute("wpa_cli -i wlan0 remove_network all", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }

  if (!wxExecute("wpa_cli -i wlan0 wps_pbc", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
  return 0;
...
int16_t WiFiClass::begin(char* ssid, const char *passphrase)
{
  wxLogMessage("connecting WiFi...SSID= %c", ssid);
  //wxLogMessage(passphrase);
  ...
  if (!wxExecute("wpa_cli -i wlan0 add_network", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
  if (!wxExecute((wxString)"wpa_cli -i wlan0 set_network 0 ssid '\"" + ssid + "\"'", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
  if (!wxExecute((wxString)"wpa_cli -i wlan0 set_network 0 psk '\"" + passphrase + "\"'", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
  if (!wxExecute("wpa_cli -i wlan0 list_networks", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
  if (!wxExecute("wpa_cli -i wlan0 enable_network 0", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
  if (!wxExecute("wpa_cli -i wlan0 reconnect", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
  if (!wxExecute("wpa_cli -i wlan0 save_config", output, wxEXEC_BLOCK))
    {
      wxLogMessage(output[0]);
    }
...
void WiFiClass::findWifiInterface()
{
  ...
  int16_t fd = socket(AF_INET, SOCK_DGRAM, 0);
  ...
  // use the first match
  struct iwreq		wrq;
  for(int16_t i = ifc.ifc_len / sizeof(struct ifreq); --i >= 0; ifr++)
    {
...
{
  wxLogMessage("WiFiClass::getStatus");
  wxArrayString output;
  wxExecute("wpa_cli -i wlan0 status", wxEXEC_BLOCK);
  String key = "";
  String value = "";
  ...
  wxLogMessage("BSSID= %s", bssid.c_str());
  wxLogMessage("SSID= %s", ssid.c_str());
  wxLogMessage("IPADDR= %s", ipaddr.c_str());
}
...
{
  wxLogMessage("WiFiClass::scanNetworks");
  wxExecute("wpa_cli -i wlan0 scan", wxEXEC_BLOCK);
  wxExecute("wpa_cli -i wlan0 scan_result", wxEXEC_BLOCK);
  wxLogMessage("WiFiClass::scanNetworks completed");
  ...
  wxLogMessage("WiFiClass::numNetworks= %i", numNetworks);

  /*for (int16_t i=0; i < numNetworks; i++){
  	wxLogMessage networkBSSID[i]);
  	wxLogMessage(",");
  	wxLogMessage networkRSSI[i]);
  	wxLogMessage(",");
  	wxLogMessage(networkSSID[i]);
  }*/
...
int16_t WiFiClass::hostByName(const char* aHostname, IPAddress& aResult)
{
  return 0;
}
