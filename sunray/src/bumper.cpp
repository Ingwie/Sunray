...
uint32_t leftPressedOnDelay = 0; // on delay timer (BUMPER_TRIGGER_DELAY) for the bumper inputs
uint32_t rightPressedOnDelay = 0;

uint32_t bumperStayActivTime = 0;    // duration, the bumper stays triggered
uint32_t lastCallBumperObstacle = 0; // last call for bumper.obstacle
  ...
  if (BUMPER_ENABLE)
    {
      ...
      // check if bumper stays triggered for a int32_t time periode (maybe blocked)
      if ((bumperRight || bumperLeft) && (BUMPER_MAX_TRIGGER_TIME > 0)){
        ...
        if ((bumperStayActivTime) > (BUMPER_MAX_TRIGGER_TIME * 1000)){ // maximum trigger time reached -> set error
          if (stateOp != OP_ERROR){
            ...
            wxLogMessage("ERROR BUMPER BLOCKED - BUMPER_MAX_TRIGGER_TIME exceeded. See config.h for further information");
