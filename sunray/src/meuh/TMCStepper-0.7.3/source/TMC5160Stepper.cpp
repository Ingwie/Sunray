...
TMC5160Stepper::TMC5160Stepper(uint8_t pinMumber, gpio_t * gpioPinCS, float RS, int8_t link) : TMC5130Stepper(pinMumber, gpioPinCS, RS, link)
{
  defaults();
}
TMC5160Stepper::TMC5160Stepper(gpio_t * gpioPinCS, float RS, int8_t link) : TMC5130Stepper(gpioPinCS, RS, link)
{
  defaults();
}
TMC5160Stepper::TMC5160Stepper(gpio_t * gpioPinCS, float RS, uint16_t pinMOSI, uint16_t pinMISO, uint16_t pinSCK, int8_t link) :
  TMC5130Stepper(gpioPinCS, RS, pinMOSI, pinMISO, pinSCK, link)
{
  defaults();
}
TMC5160Stepper::TMC5160Stepper(gpio_t * gpioPinCS, uint16_t pinMOSI, uint16_t pinMISO, uint16_t pinSCK, int8_t link) :
  TMC5130Stepper(gpioPinCS, default_RS, pinMOSI, pinMISO, pinSCK, link)
{
  defaults();
}
...
// R+WC: ENC_STATUS
uint8_t TMC5160Stepper::ENC_STATUS()
{
  return read(ENC_STATUS_t::address);
}
...
// W: ENC_DEVIATION
uint32_t TMC5160Stepper::ENC_DEVIATION()
{
  return ENC_DEVIATION_register.sr;
}
...
uint8_t TMC5160Stepper::pwm_ofs_auto()
{
  PWM_AUTO_t r{0};
  r.sr = PWM_AUTO();
  return r.pwm_ofs_auto;
}
uint8_t TMC5160Stepper::pwm_grad_auto()
{
  PWM_AUTO_t r{0};
  r.sr = PWM_AUTO();
...
// pin CS OFF
void TMC5160Stepper::CS_OFF()
{
  GpioPinWrite(_pinCS, 1);
}

// pin CS ON
void TMC5160Stepper::CS_ON()
{
  GpioPinWrite(_pinCS, 0);
}
