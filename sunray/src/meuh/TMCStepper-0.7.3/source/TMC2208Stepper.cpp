...
__attribute__((weak))
int16_t TMC2208Stepper::available()
{
  int16_t out = 0;
#if SW_CAPABLE_PLATFORM
  if (SWSerial != nullptr)
...
uint64_t TMC2208Stepper::_sendDatagram(uint8_t datagram[], const uint8_t len, uint16_t timeout)
{
  ...
  for(int16_t i=0; i<=len; i++) serial_write(datagram[i]);
...
{
  FACTORY_CONF_register.fclktrim = B;
  write(FACTORY_CONF_register.address, FACTORY_CONF_register.sr);
}
void TMC2208Stepper::ottrim(uint8_t B)
{
  FACTORY_CONF_register.ottrim = B;
  write(FACTORY_CONF_register.address, FACTORY_CONF_register.sr);
}
