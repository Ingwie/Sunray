...
TMC5130Stepper::TMC5130Stepper(gpio_t * gpioPinCS, float RS, uint16_t pinMOSI, uint16_t pinMISO, uint16_t pinSCK, int8_t link):
  TMC2160Stepper(gpioPinCS, RS, pinMOSI, pinMISO, pinSCK, link)
{
  defaults();
}
TMC5130Stepper::TMC5130Stepper(gpio_t * gpioPinCS, uint16_t pinMOSI, uint16_t pinMISO, uint16_t pinSCK, int8_t link) :
  TMC2160Stepper(gpioPinCS, default_RS, pinMOSI, pinMISO, pinSCK, link)
{
  defaults();
}
...
bool    TMC5130Stepper::encb_dcen_cfg4()
{
  TMC5130_n::IOIN_t r{0};
  r.sr = IOIN();
  return r.encb_dcen_cfg4;
}
bool    TMC5130Stepper::enca_dcin_cfg5()
{
  TMC5130_n::IOIN_t r{0};
  r.sr = IOIN();
  return r.enca_dcin_cfg5;
}
bool    TMC5130Stepper::drv_enn_cfg6()
{
  TMC5130_n::IOIN_t r{0};
  r.sr = IOIN();
  return r.drv_enn_cfg6;
}
...
{
  return read(RAMPMODE_register.address);
}
...
{
  return read(XACTUAL_register.address);
}
...
{
  return read(XLATCH_t::address);
}
///////////////////////////////////////////////////////////////////////////////////////
// RW: X_ENC
int32_t TMC5130Stepper::X_ENC()
{
  return read(X_ENC_t::address);
}
...
{
  return read(ENC_LATCH_t::address);
}
