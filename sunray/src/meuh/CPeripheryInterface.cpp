...
void SPIC::begin()
{
  if (CPERIPH_VERBOSE) wxLogMessage("Create SPI");
}

void SPIC::beginTransaction(SPISettings settings)
{
  if (CPERIPH_VERBOSE) wxLogMessage("SPI beginTransaction");
}

uint8_t SPIC::transfer(uint8_t data)
{
  if (CPERIPH_VERBOSE) wxLogMessage("SPI transfer %#x", data);
  return 0x00;
}

void SPIC::transfertTmcFrame(tmcFrame * frame)
{
  if (CPERIPH_VERBOSE) wxLogMessage("SPI transfertTmcFrame %#x %#x", frame->firstByte, frame->datas);
}
...
void I2CC::begin()
{
  if (CPERIPH_VERBOSE) wxLogMessage("I2C begin");
}

bool I2CC::transfer(struct i2c_msg* msgs, size_t msgsnum)
{
  if (CPERIPH_VERBOSE) wxLogMessage("I2C transfert %#x len %i", msgs->addr, msgs->len);
  return true;
}
