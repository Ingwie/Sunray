...
static int16_t _pwm_error(pwm_t *pwm, int16_t code, int16_t c_errno, const char *fmt, ...)
{
  va_list ap;
...
pwm_t *pwm_new(void)
{
  pwm_t *pwm = (pwm_t *)calloc(1, sizeof(pwm_t));
  if (pwm == NULL)
    return NULL;
...
int16_t pwm_open(pwm_t *pwm, uint16_t chip, uint16_t channel)
{
...
int16_t pwm_enable(pwm_t *pwm)
{
  return pwm_set_enabled(pwm, true);
}

int16_t pwm_disable(pwm_t *pwm)
{
  return pwm_set_enabled(pwm, false);
}

int16_t pwm_close(pwm_t *pwm)
{
  char path[P_PATH_MAX];
  char buf[16];
  int16_t len;
...
static int16_t pwm_read_attribute(pwm_t *pwm, const char *name, char *buf, size_t len)
{
  char path[P_PATH_MAX];
  int16_t fd, ret;
...
static int16_t pwm_write_attribute(pwm_t *pwm, const char *name, const char *buf, size_t len)
{
  char path[P_PATH_MAX];
  int16_t fd;
...
int16_t pwm_get_enabled(pwm_t *pwm, bool *enabled)
{
  char buf[2];
  int16_t ret;
...
int16_t pwm_get_period_ns(pwm_t *pwm, uint64_t *period_ns)
{
  char buf[32];
  int16_t ret;
  uint64_t value;
...
int16_t pwm_get_duty_cycle_ns(pwm_t *pwm, uint64_t *duty_cycle_ns)
{
  char buf[32];
  int16_t ret;
  uint64_t value;
...
int16_t pwm_get_period(pwm_t *pwm, double *period)
{
  int16_t ret;
  uint64_t period_ns;
...
int16_t pwm_get_duty_cycle(pwm_t *pwm, double *duty_cycle)
{
  int16_t ret;
...
int16_t pwm_get_frequency(pwm_t *pwm, double *frequency)
{
  int16_t ret;
  uint64_t period_ns;
...
int16_t pwm_get_polarity(pwm_t *pwm, pwm_polarity_t *polarity)
{
  int16_t ret;
  char buf[16];
...
int16_t pwm_set_enabled(pwm_t *pwm, bool enabled)
{
  return pwm_write_attribute(pwm, "enable", enabled ? "1\n" : "0\n", 2);
}

int16_t pwm_set_period_ns(pwm_t *pwm, uint64_t period_ns)
{
  char buf[32];
  int16_t len;
  int16_t ret;
...
int16_t pwm_set_duty_cycle_ns(pwm_t *pwm, uint64_t duty_cycle_ns)
{
  char buf[32];
  int16_t len;
...
int16_t pwm_set_period(pwm_t *pwm, double period)
{
...
int16_t pwm_set_duty_cycle(pwm_t *pwm, double duty_cycle)
{
  uint64_t duty_cycle_ns;
...
int16_t pwm_set_frequency(pwm_t *pwm, double frequency)
{
...
int16_t pwm_set_polarity(pwm_t *pwm, pwm_polarity_t polarity)
{
  const char *buf;
...
uint16_t pwm_chip(pwm_t *pwm)
{
  return pwm->chip;
}

uint16_t pwm_channel(pwm_t *pwm)
{
  return pwm->channel;
}

int16_t pwm_tostring(pwm_t *pwm, char *str, size_t len)
{
  double period;
  char period_str[16];
  double duty_cycle;
