...
static int16_t _led_error(led_t *led, int16_t code, int16_t c_errno, const char *fmt, ...)
{
  va_list ap;
...
int16_t led_open(led_t *led, const char *name)
{
...
int16_t led_read(led_t *led, bool *value)
{
  int16_t ret;
  uint16_t brightness;
...
int16_t led_write(led_t *led, bool value)
{
  return led_set_brightness(led, value ? led->max_brightness : 0);
}

int16_t led_close(led_t *led)
{
  (void)led;
  return 0;
}
...
int16_t led_get_brightness(led_t *led, uint16_t *brightness)
{
  char led_path[P_PATH_MAX];
  char buf[16];
  int16_t fd, ret;
...
int16_t led_get_max_brightness(led_t *led, uint16_t *max_brightness)
{
  char led_path[P_PATH_MAX];
  char buf[16];
  int16_t fd, ret;
...
int16_t led_set_brightness(led_t *led, uint16_t brightness)
{
  char led_path[P_PATH_MAX];
  char buf[16];
  int16_t fd, len;
...
int16_t led_name(led_t *led, char *str, size_t len)
{
  if (!len)
    return 0;
...
int16_t led_tostring(led_t *led, char *str, size_t len)
{
  uint16_t brightness;
