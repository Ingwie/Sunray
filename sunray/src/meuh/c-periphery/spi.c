...
static int16_t _spi_error(spi_t *spi, int16_t code, int16_t c_errno, const char *fmt, ...)
{
  va_list ap;
...
int16_t spi_open(spi_t *spi, const char *path, uint16_t mode, uint32_t max_speed)
{
  return spi_open_advanced(spi, path, mode, max_speed, MSB_FIRST, 8, 0);
}

int16_t spi_open_advanced(spi_t *spi, const char *path, uint16_t mode, uint32_t max_speed, spi_bit_order_t bit_order, uint8_t bits_per_word, uint8_t extra_flags)
{
  return spi_open_advanced2(spi, path, mode, max_speed, bit_order, bits_per_word, extra_flags);
}

int16_t spi_open_advanced2(spi_t *spi, const char *path, uint16_t mode, uint32_t max_speed, spi_bit_order_t bit_order, uint8_t bits_per_word, uint32_t extra_flags)
{
  uint32_t data32;
  uint8_t data8;
...
int16_t spi_transfer(spi_t *spi, const uint8_t *txbuf, uint8_t *rxbuf, size_t len)
{
...
int16_t spi_get_mode(spi_t *spi, uint16_t *mode)
{
  uint8_t data8;
...
int16_t spi_get_max_speed(spi_t *spi, uint32_t *max_speed)
{
  uint32_t data32;
...
int16_t spi_get_bit_order(spi_t *spi, spi_bit_order_t *bit_order)
{
  uint8_t data8;
...
int16_t spi_get_bits_per_word(spi_t *spi, uint8_t *bits_per_word)
{
  uint8_t data8;
...
int16_t spi_get_extra_flags(spi_t *spi, uint8_t *extra_flags)
{
  uint8_t data8;
...
int16_t spi_get_extra_flags32(spi_t *spi, uint32_t *extra_flags)
{
...
int16_t spi_set_mode(spi_t *spi, uint16_t mode)
{
  uint8_t data8;
...
int16_t spi_set_bit_order(spi_t *spi, spi_bit_order_t bit_order)
{
  uint8_t data8;
...
int16_t spi_set_extra_flags(spi_t *spi, uint8_t extra_flags)
{
  uint8_t data8;
...
int16_t spi_set_extra_flags32(spi_t *spi, uint32_t extra_flags)
{
...
int16_t spi_set_max_speed(spi_t *spi, uint32_t max_speed)
{
...
int16_t spi_set_bits_per_word(spi_t *spi, uint8_t bits_per_word)
{
...
int16_t spi_tostring(spi_t *spi, char *str, size_t len)
{
  uint16_t mode;
  char mode_str[2];
  uint32_t max_speed;
...
int16_t spi_errno(spi_t *spi)
{
  return spi->error.c_errno;
}

int16_t spi_fd(spi_t *spi)
{
  return spi->fd;
}
