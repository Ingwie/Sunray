...
static int16_t _serial_error(serial_t *serial, int16_t code, int16_t c_errno, const char *fmt, ...)
{
  va_list ap;
...
static int16_t _serial_baudrate_to_bits(uint32_t baudrate)
{
  switch (baudrate)
    {
    case 50:
...
static uint32_t _serial_bits_to_baudrate(uint32_t bits)
{
  switch (bits)
    {
    case B0:
...
int16_t serial_open_advanced(serial_t *serial, const char *path, uint32_t baudrate, uint16_t databits, serial_parity_t parity, uint16_t stopbits, bool xonxoff, bool rtscts)
{
...
int16_t serial_set_vtime(serial_t *serial, float vtime)
{
  ...
  termios_settings.c_cc[VTIME] = ((uint16_t)(vtime * 10));
