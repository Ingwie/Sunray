...
static int16_t _mmio_error(mmio_t *mmio, int16_t code, int16_t c_errno, const char *fmt, ...)
{
  va_list ap;
...
{
  return (mmio_t *)calloc(1, sizeof(mmio_t));
}
...
int16_t mmio_open(mmio_t *mmio, uintptr_t base, size_t size)
{
  return mmio_open_advanced(mmio, base, size, "/dev/mem");
}

int16_t mmio_open_advanced(mmio_t *mmio, uintptr_t base, size_t size, const char *path)
{
  int16_t fd;
...
int16_t mmio_read32(mmio_t *mmio, uintptr_t offset, uint32_t *value)
{
...
int16_t mmio_read16(mmio_t *mmio, uintptr_t offset, uint16_t *value)
{
  offset += (mmio->base - mmio->aligned_base);
...
int16_t mmio_read8(mmio_t *mmio, uintptr_t offset, uint8_t *value)
{
  offset += (mmio->base - mmio->aligned_base);
...
int16_t mmio_read(mmio_t *mmio, uintptr_t offset, uint8_t *buf, size_t len)
{
  offset += (mmio->base - mmio->aligned_base);
...
int16_t mmio_write32(mmio_t *mmio, uintptr_t offset, uint32_t value)
{
  offset += (mmio->base - mmio->aligned_base);
  if ((offset+4) > mmio->aligned_size)
    return _mmio_error(mmio, MMIO_ERROR_ARG, 0, "Offset out of bounds");
...
int16_t mmio_write16(mmio_t *mmio, uintptr_t offset, uint16_t value)
{
  offset += (mmio->base - mmio->aligned_base);
  if ((offset+2) > mmio->aligned_size)
    return _mmio_error(mmio, MMIO_ERROR_ARG, 0, "Offset out of bounds");
...
int16_t mmio_write8(mmio_t *mmio, uintptr_t offset, uint8_t value)
{
  offset += (mmio->base - mmio->aligned_base);
  if ((offset+1) > mmio->aligned_size)
    return _mmio_error(mmio, MMIO_ERROR_ARG, 0, "Offset out of bounds");
...
int16_t mmio_write(mmio_t *mmio, uintptr_t offset, const uint8_t *buf, size_t len)
{
  offset += (mmio->base - mmio->aligned_base);
  if ((offset+len) > mmio->aligned_size)
    return _mmio_error(mmio, MMIO_ERROR_ARG, 0, "Offset out of bounds");
...
int16_t mmio_close(mmio_t *mmio)
{
  if (!mmio->ptr)
    return 0;
...
int16_t mmio_tostring(mmio_t *mmio, char *str, size_t len)
{
...
int16_t mmio_errno(mmio_t *mmio)
{
