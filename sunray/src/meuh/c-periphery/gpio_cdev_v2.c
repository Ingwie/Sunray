...
static int 16_t _gpio_cdev_reopen(gpio_t *gpio, gpio_direction_t direction, gpio_edge_t edge, gpio_bias_t bias, gpio_drive_t drive, bool inverted)
{
  uint32_t flags = 0;
...
static int16_t gpio_cdev_read(gpio_t *gpio, bool *value)
{
...
static int16_t gpio_cdev_write(gpio_t *gpio, bool value)
{
  struct gpio_v2_line_values line_values = {0, 1};
...
static int16_t gpio_cdev_read_event(gpio_t *gpio, gpio_edge_t *edge, uint64_t *timestamp)
{
...
static int16_t gpio_cdev_poll(gpio_t *gpio, int16_t timeout_ms)
{
  struct pollfd fds[1];
  int16_t ret;
...
static int16_t gpio_cdev_close(gpio_t *gpio)
{
  /* Close line fd */
  if (gpio->u.cdev.line_fd >= 0)
    {
...
static int16_t gpio_cdev_get_direction(gpio_t *gpio, gpio_direction_t *direction)
{
  *direction = gpio->u.cdev.direction;
  return 0;
}

static int16_t gpio_cdev_get_edge(gpio_t *gpio, gpio_edge_t *edge)
{
  *edge = gpio->u.cdev.edge;
  return 0;
}

static int16_t gpio_cdev_get_bias(gpio_t *gpio, gpio_bias_t *bias)
{
  *bias = gpio->u.cdev.bias;
  return 0;
}

static int16_t gpio_cdev_get_drive(gpio_t *gpio, gpio_drive_t *drive)
{
  *drive = gpio->u.cdev.drive;
  return 0;
}

static int16_t gpio_cdev_get_inverted(gpio_t *gpio, bool *inverted)
{
  *inverted = gpio->u.cdev.inverted;
  return 0;
}

static int16_t gpio_cdev_set_direction(gpio_t *gpio, gpio_direction_t direction)
{
...
static int16_t gpio_cdev_set_edge(gpio_t *gpio, gpio_edge_t edge)
{
...
static int16_t gpio_cdev_set_bias(gpio_t *gpio, gpio_bias_t bias)
{
...
static int16_t gpio_cdev_set_drive(gpio_t *gpio, gpio_drive_t drive)
{
...
static int16_t gpio_cdev_set_inverted(gpio_t *gpio, bool inverted)
{
...
static uint16_t gpio_cdev_line(gpio_t *gpio)
{
  return gpio->u.cdev.line;
}

static int16_t gpio_cdev_fd(gpio_t *gpio)
{
  return gpio->u.cdev.line_fd;
}

static int16_t gpio_cdev_name(gpio_t *gpio, char *str, size_t len)
{
...
static int16_t gpio_cdev_label(gpio_t *gpio, char *str, size_t len)
{
  struct gpio_v2_line_info line_info = {0};
...
static int16_t gpio_cdev_chip_fd(gpio_t *gpio)
{
  return gpio->u.cdev.chip_fd;
}

static int16_t gpio_cdev_chip_name(gpio_t *gpio, char *str, size_t len)
{
...
static int16_t gpio_cdev_chip_label(gpio_t *gpio, char *str, size_t len)
{
  struct gpiochip_info chip_info = {0};
...
static int16_t gpio_cdev_tostring(gpio_t *gpio, char *str, size_t len)
{
...
int16_t gpio_open_advanced(gpio_t *gpio, const char *path, uint16_t line, const gpio_config_t *config)
{
  int16_t ret, fd;
...
int16_t gpio_open_name_advanced(gpio_t *gpio, const char *path, const char *name, const gpio_config_t *config)
{
  int16_t fd;
...
int16_t gpio_open(gpio_t *gpio, const char *path, uint16_t line, gpio_direction_t direction)
{
...
int16_t gpio_open_name(gpio_t *gpio, const char *path, const char *name, gpio_direction_t direction)
{
  gpio_config_t config =
  {
    .direction = direction,
