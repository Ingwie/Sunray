...
static int16_t gpio_sysfs_close(gpio_t *gpio)
{
  char buf[16];
  int16_t len, fd;
...
static int16_t gpio_sysfs_read(gpio_t *gpio, bool *value)
{
  char buf[2];
...
static int16_t gpio_sysfs_write(gpio_t *gpio, bool value)
{
...
static int16_t gpio_sysfs_read_event(gpio_t *gpio, gpio_edge_t *edge, uint64_t *timestamp)
{
  (void)edge;
  (void)timestamp;
...
static int16_t gpio_sysfs_poll(gpio_t *gpio, int16_t timeout_ms)
{
  struct pollfd fds[1];
  int16_t ret;
...
static int16_t gpio_sysfs_set_direction(gpio_t *gpio, gpio_direction_t direction)
{
...
static int16_t gpio_sysfs_get_direction(gpio_t *gpio, gpio_direction_t *direction)
{
  char gpio_path[P_PATH_MAX];
  char buf[8];
  int16_t fd, ret;
...
static int16_t gpio_sysfs_set_edge(gpio_t *gpio, gpio_edge_t edge)
{
  char gpio_path[P_PATH_MAX];
  const char *buf;
  int16_t fd;
...
static int16_t gpio_sysfs_get_edge(gpio_t *gpio, gpio_edge_t *edge)
{
  char gpio_path[P_PATH_MAX];
  char buf[16];
  int16_t fd, ret;
...
static int16_t gpio_sysfs_set_bias(gpio_t *gpio, gpio_bias_t bias)
{
  (void)bias;
  return _gpio_error(gpio, GPIO_ERROR_UNSUPPORTED, 0, "GPIO of type sysfs does not support line bias attribute");
}

static int16_t gpio_sysfs_get_bias(gpio_t *gpio, gpio_bias_t *bias)
{
  (void)bias;
  return _gpio_error(gpio, GPIO_ERROR_UNSUPPORTED, 0, "GPIO of type sysfs does not support line bias attribute");
}

static int16_t gpio_sysfs_set_drive(gpio_t *gpio, gpio_drive_t drive)
{
  (void)drive;
  return _gpio_error(gpio, GPIO_ERROR_UNSUPPORTED, 0, "GPIO of type sysfs does not support line drive attribute");
}

static int16_t gpio_sysfs_get_drive(gpio_t *gpio, gpio_drive_t *drive)
{
  (void)drive;
  return _gpio_error(gpio, GPIO_ERROR_UNSUPPORTED, 0, "GPIO of type sysfs does not support line drive attribute");
}

static int16_t gpio_sysfs_set_inverted(gpio_t *gpio, bool inverted)
{
  char gpio_path[P_PATH_MAX];
...
static int16_t gpio_sysfs_get_inverted(gpio_t *gpio, bool *inverted)
{
  char gpio_path[P_PATH_MAX];
  char buf[4];
  int16_t fd, ret;
...
static uint16_t gpio_sysfs_line(gpio_t *gpio)
{
  return gpio->u.sysfs.line;
}

static int16_t gpio_sysfs_fd(gpio_t *gpio)
{
  return gpio->u.sysfs.line_fd;
}

static int16_t gpio_sysfs_name(gpio_t *gpio, char *str, size_t len)
{
  (void)gpio;
  if (len)
    str[0] = '\0';
...
static int16_t gpio_sysfs_label(gpio_t *gpio, char *str, size_t len)
{
  (void)gpio;
  if (len)
    str[0] = '\0';
...
static int16_t gpio_sysfs_chip_fd(gpio_t *gpio)
{
  return _gpio_error(gpio, GPIO_ERROR_UNSUPPORTED, 0, "GPIO of type sysfs has no chip fd");
}

static int16_t gpio_sysfs_chip_name(gpio_t *gpio, char *str, size_t len)
{
  int16_t ret;
  char gpio_path[P_PATH_MAX];
...
static int16_t gpio_sysfs_chip_label(gpio_t *gpio, char *str, size_t len)
{
  char gpio_path[P_PATH_MAX];
  char chip_name[32];
  int16_t fd, ret;
...
static int16_t gpio_sysfs_tostring(gpio_t *gpio, char *str, size_t len)
{
  ...
  if (stat(gpio_path, &stat_buf) < 0)
    {
      ...
      for (retry_count = 0; retry_count < GPIO_SYSFS_OPEN_RETRIES; retry_count++)
        {
          int16_t ret = stat(gpio_path, &stat_buf);
          if (ret == 0)
            {
              exported = true;
              break;
