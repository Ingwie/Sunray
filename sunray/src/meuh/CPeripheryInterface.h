...
#define CPERIPH_VERBOSE false

//  GPIO

#define SetGpioPin(name, direction) \
 if (CPERIPH_VERBOSE) wxLogMessage("Create new GPIO %i", name##_Number);

#define GpioPinWrite(name, value) \
 if (CPERIPH_VERBOSE) wxLogMessage("Write a GPIO %b", value);

#define  GpioPinRead(name) false
//void GpioSetEdge(gpio_t * name, gpio_edge edge);
//void GpioReadEvent(gpio_t * name, gpio_edge edge, uint64_t * timestamp);
...
#define SetNewPwm(pwm, num) \
 if (CPERIPH_VERBOSE) wxLogMessage("Create new PWM %i", num);

#define PwmDisable(pwm) \
 if (CPERIPH_VERBOSE) wxLogMessage("Disable a PWM");

#define PwmSetFrequency(pwm, frequency) \
 if (CPERIPH_VERBOSE) wxLogMessage("Set PWM frequency %.2f", (float)frequency);

#define PwmSetDutyCycle(pwm, dutycycle) \
 if (CPERIPH_VERBOSE) wxLogMessage("Set PWM dutycycle %.2f", (float)dutycycle);

#define PwmSetPolarity(pwm, polarity) \
 if (CPERIPH_VERBOSE) wxLogMessage("Set PWM polarity");
