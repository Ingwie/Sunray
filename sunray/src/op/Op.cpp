...
{
  /*if (&anOp == NULL){
      wxLogMessage("==> ERROR Op::changeOp: invalid op=NULL!");
  }*/
...
{
  wxLogMessage("changeOperationType");
  if (activeOp == NULL)
    {
      wxLogMessage("ERROR Op::changeOperationType - activeOp=NULL");
      return;
    }
  switch (op)
    {
...
OperationType Op::getGoalOperationType()
{
  ...
  wxLogMessage("ERROR: Op::getGoalOperationType: invalid goal op!");
  return OP_ERROR;
}
  ...
  while (goalOp->nextOp != NULL)
    {
      ...
      if (chainCounter > 10)
        {
          wxLogMessage("ERROR Op::getGoalOp: invalid op chain (infinity)");
          break;
        }
    }
  return goalOp;
  ...
  while (goalOp->nextOp != NULL)
    {
      ...
      if (chainCounter > 10)
        {
          wxLogMessage("ERROR Op::getOpChain: invalid op chain (infinity)");
          break;
        }
    }
  return opChain;
  ...
  if (shouldStop)
    {
      if (newActiveOp == NULL)
        {
          wxLogMessage("ERROR Op::checkStop: invalid newActiveOp=NULL");
          return;
        }
      end();
      ...
      wxLogMessage("==> changeOp:");
      activeOp->OpChain =  activeOp->getOpChain();
      wxLogMessage("%S", activeOp->OpChain.c_str());
