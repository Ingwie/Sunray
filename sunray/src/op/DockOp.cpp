...
void DockOp::begin()
{
  ...
  wxLogMessage("OP_DOCK");
  wxLogMessage(" initiatedByOperator=%B  dockReasonRainTriggered=%B", initiatedByOperator, dockReasonRainTriggered);
  ...
  if (routingFailed)
    {
      ...
      if (mapRoutingFailedCounter > 60)
        {
          wxLogMessage("error: too many routing errors!");
          stateSensor = SENS_MAP_NO_ROUTE;
          changeOp(errorOp);
        }
      else
...
{
  wxLogMessage("DockOp::onTargetReached");
...
{
  wxLogMessage("error: rotation error due to obstacle!");
  statMowObstacles++;
...
void DockOp::onObstacle()
{
  if (battery.chargerConnected())
    {
      wxLogMessage("triggerObstacle: ignoring, because charger connected");
      return;
    }
