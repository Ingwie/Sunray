  ...
  if (maps.startMowing(stateX, stateY))
    {
      if (maps.nextPoint(true, stateX, stateY))
        ...
        {
          error = true;
          wxLogMessage("error: no waypoints!");
          //op = stateOp;
        }
    }
  else error = true;
  ...
  if (routingFailed)
    {
      ...
      if (mapRoutingFailedCounter > 60)
        {
          wxLogMessage("error: too many map routing errors!");
          stateSensor = SENS_MAP_NO_ROUTE;
          changeOp(errorOp);
        }
      else
  ...
  if (timetable.shouldAutostopNow())
    {
      if (DOCKING_STATION)
        {
          wxLogMessage("TIMETABLE - DOCKING");
          dockOp.setInitiatedByOperator(false);
          changeOp(dockOp);
        }
      else
        {
          wxLogMessage("TIMETABLE - IDLE");
          changeOp(idleOp);
        }
    }
}
...
void MowOp::onTempOutOfRangeTriggered()
{
  if (DOCKING_STATION)
    {
      wxLogMessage("TEMP OUT-OF-RANGE TRIGGERED");
...
{
  wxLogMessage("BATTERY LOW TRIGGERED - DOCKING");
  dockOp.setInitiatedByOperator(false);
  changeOp(dockOp);
}
...
{
  wxLogMessage("triggerObstacleRotation");
  statMowObstacles++;
  if ((OBSTACLE_AVOIDANCE) && (maps.wayMode != WAY_DOCK))
    {
...
void MowOp::onOdometryError()
{
  if (ENABLE_ODOMETRY_ERROR_DETECTION)
    {
      wxLogMessage("error: odometry error!");
  ...
  if (ENABLE_OVERLOAD_DETECTION)
    {
      if (motor.motorOverloadDuration > 20000)
        {
          wxLogMessage("error: motor overload!");
...
void MowOp::onMotorError()
{
  if (ENABLE_FAULT_OBSTACLE_AVOIDANCE)
      ...
      if (motor.motorError)
        {
          ...
          motorErrorCounter++;
          wxLogMessage("MowOp::onMotorError motorErrorCounter=%",motorErrorCounter);
          ...
          wxLogMessage("error: motor error - giving up!");
          motorErrorCounter = 0;
          stateSensor = SENS_MOTOR_ERROR;
          changeOp(errorOp);
          return;
    ...
    {
      wxLogMessage("no obstacle avoidance activated on motor errors, giving up");
      stateSensor = SENS_MOTOR_ERROR;
      changeOp(errorOp);
      return;
    }
