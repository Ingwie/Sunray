...
uint32_t stateLeftTicks = 0;
uint32_t stateRightTicks = 0;
...
int16_t imuCalibrationSeconds = 0;
uint32_t nextImuCalibrationSecond = 0;
uint32_t nextDumpTime = 0;
      ...
      if (counter > 5)
        {
          ...
          wxLogMessage("ERROR IMU not found");
  ...
  while (true)
    {
      ...
      wxLogMessage("Unable to communicate with IMU.");
      wxLogMessage("Check connections, and try again.");
...
void dumpImuTilt()
{
  ...
  wxLogMessage("IMU tilt: ypr= %f , %f, %f rollChange= %f  pitchChange= %f", \
               imuDriver.yaw/PI*180.0, imuDriver.pitch/PI*180.0, imuDriver.roll/PI*180.0, rollChange/PI*180.0, pitchChange/PI*180.0);
}
...
void readIMU()
{
  ...
  // check time for I2C access : if too int32_t, there's an I2C issue and we need to restart I2C bus...
  ...
  if ((duration > 60) || (millis() > imuDataTimeout))
    {
      if (millis() > imuDataTimeout)
        {
          wxLogMessage("ERROR IMU data timeout: %i (check RTC battery if problem persists)", millis()-imuDataTimeout);
        }
      else
        {
          wxLogMessage("ERROR IMU timeout: %i (check RTC battery if problem persists)", duration);
        }
      stateSensor = SENS_IMU_TIMEOUT;
  ...
  if (avail)
    {
      ...
      //wxLogMessage(rollChange/PI*180.0);
      //wxLogMessage(",");
      //wxLogMessage(pitchChange/PI*180.0);
      ...
      //wxLogMessage(imuDriver.yaw / PI * 180.0);
      ...
      //wxLogMessage(imuDriver.yaw);
      //wxLogMessage(",");
      //wxLogMessage(stateDeltaIMU/PI*180.0);
      //
...
// to fusion GPS heading (int32_t-term) and IMU heading (short-term)
...
{
  int32_t leftDelta = motor.motorLeftTicks-stateLeftTicks;
  int32_t rightDelta = motor.motorRightTicks-stateRightTicks;
          ...
          if (distGPS > 0.3)
            {
              gpsJump = true;
              statGPSJumps++;
              wxLogMessage("GPS jump: %f", distGPS);
            }
          resetLastPos = false;
          lastPosN = posN;
          lastPosE = posE;
  ...
  //wxLogMessage(stateDelta / PI * 180.0);
  stateDeltaIMU = 0;
  ...
  //wxLogMessage(stateDeltaSpeedLP/PI*180.0);
  ...
  if (imuDriver.imuFound)
    {
      ...
      //wxLogMessage(diffIMUWheelYawSpeedLP/PI*180.0);
      //wxLogMessage(stateDeltaSpeedIMU/PI*180.0);
      //wxLogMessage(",");
      //wxLogMessage(stateDeltaSpeedWheels/PI*180.0);
    }
}
