...
#define NewPingConvert(echoTime, conversionFactor) (max(((uint16_t)echoTime + conversionFactor / 2) / conversionFactor, (echoTime ? 1 : 0)))



RunningMedian<uint16_t, 9> sonarLeftMeasurements;
RunningMedian<uint16_t, 9> sonarRightMeasurements;
RunningMedian<uint16_t, 9> sonarCenterMeasurements;

volatile uint32_t startTime = 0;
volatile uint32_t echoTime = 0;
volatile uint32_t echoDuration = 0;
volatile uint8_t sonarIdx = 0;
bool added = false;
uint32_t timeoutTime = 0;
uint32_t nextEvalTime = 0;


#ifdef SONAR_INSTALLED

// HC-SR04 ultrasonic sensor driver (2cm - 400cm)
void startHCSR04(int16_t triggerPin, int16_t aechoPin)
{
  uint16_t uS;
...
uint16_t Sonar::convertCm(uint16_t echoTime)
{
