...
#define pin_cur_pol_Number        4 // was pin_aio_ck
...
#define BAT_POT_FACTOR          15.130 // POT_FACTOR(22000.0f, 300000.0f) // todo measure real values
#define CHARGE_POT_FACTOR       15.30 // POT_FACTOR(22000.0f, 300000.0f)
#define VCC_POT_FACTOR          2.552   // POT_FACTOR(4700.0f, 6800.0f)
#define ADC_FILTER_VAL          0.5 // SIMPLE FILTER
#define ADC_FILTER(val, raw)    val = (raw * ADC_FILTER_VAL + val * (1 - ADC_FILTER_VAL))

// MAX471 I/2000 v output DIV2 by 1K/1K pot divisor
#define MAX471_VOLTS_TO_AMPS(x) (float)(x * 2.07)
...
public:
  ...
  int16_t lastLeftPwm;
  int16_t lastRightPwm;
  int16_t lastMowPwm;
  uint64_t encoderTicksMow;
  //uint32_t encoderTicksLeft;
  //uint32_t encoderTicksRight;
  ...
  //void requestMotorPwm(int16_t leftPwm, int16_t rightPwm, int16_t mowPwm);
  void updateCpuTemperature();
  void updateWifiConnectionState();
  bool setFanPowerTune(float temp);
  float readAdcChannel(ADS1115_MUX channel);
  void exitApp(int16_t error);
  void tmcLogicOff();
  void tmcLogicOn();
  void relayStopAll();
  void relayPowerOn();
...
protected:
  String cmd;
  //String cmdResponse;
  //uint32_t nextMotorTime;
  //uint32_t nextSummaryTime;
  //uint32_t nextConsoleTime;
  uint32_t nextTempTime;
  uint32_t nextWifiTime;
  //int16_t cmdMotorCounter;
  //int16_t cmdSummaryCounter;
  //int16_t cmdMotorResponseCounter;
  //int16_t cmdSummaryResponseCounter;
  //void processComm();
...
public:
  ...
  void begin() override;
  void run() override;
  void setMotorPwm(int16_t leftPwm, int16_t rightPwm, int16_t mowPwm) override;
  void printTmcError(TMC5160_DRV_STATUS_t status);
  void checkTmcState(TMC5160Stepper &stepper, TMC5160_DRV_STATUS_t &status, bool &errorBool, float &current, int16_t &lastPwm);
  ...
  void getMotorEncoderTicks(int16_t &leftTicks, int16_t &rightTicks, int16_t &mowTicks) override;
protected:
  uint8_t L_SpiStatus;
  uint8_t R_SpiStatus;
  bool L_MotorFault;
...
public:
  ...
  void tone(int16_t freq) override;
};
...
public:
  bool simGpsJump;
  MeuhRobotDriver &simRobot;
  SimGpsDriver(MeuhRobotDriver &sr);
