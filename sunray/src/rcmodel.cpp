...
volatile uint32_t PPM_start_lin = 0;
volatile uint32_t PPM_end_lin = 0;
volatile uint32_t PPM_start_ang = 0;
volatile uint32_t PPM_end_ang = 0 ;
...
{
#ifdef RCMODEL_ENABLE
  wxLogMessage("RCModel enabled in config");
  lin_PPM = 0;
  linearPPM = 0;
  ang_PPM = 0;
  angularPPM = 0;
...
#endif
#else
  wxLogMessage("RCModel disabled in config");
#endif
}
  ...
  if (stateButton == 3)                                            // 3 button beeps
    {
      ...
      if (RC_Mode)                                                          // R/C-Mode ist aktiv
        {
          wxLogMessage("button mode 3 - RC Mode ON");
      ...
      if (!RC_Mode)
        {
          wxLogMessage("button mode 3 - RC Mode OFF");                                      // R/C-Mode inaktiv
      ...
      if (t >= nextOutputTime)
        {
          ...
          wxLogMessage("RC: linearPPM= ");
          wxLogMessage(linearPPM);
          wxLogMessage("  angularPPM= ");
          wxLogMessage(angularPPM);
        }
#endif
