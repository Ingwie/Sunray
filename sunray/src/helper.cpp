...
void relativeLL(double lat1, double lon1, double lat2, double lon2, float &n, float &e)
{
  ...
  int16_t slat = sign(lat2-lat1);
  int16_t slon = sign(lon2-lon1);
  n = slat * distanceLL(lat1, lon1, lat2, lon1);
  e = slon * distanceLL(lat1, lon1, lat1, lon2);
  if (false)
    {
      wxLogMessage("relativeLL lat1=%f lat2=%f n=%f relativeLL lon1=%f lon2=%f e=%f", lat1, lat2, n, lon1, lon2,e);
    }
}
...
void printFloat(float v)
{
  binaryLongOrFloat b;
  b.floatingPoint = v;
  wxLogMessage("%i", b.binary[3]);
  wxLogMessage("%i", b.binary[2]);
  wxLogMessage("%i", b.binary[1]);
  wxLogMessage("%i", b.binary[0]);
}
