...
public:
  ...
  int32_t crc();
  bool read(wxFile &file);
  bool write(wxFile &file);
};
...
public:
  ...
  int16_t numPoints;
  Polygon();
  Polygon(int16_t aNumPoints);
  ~Polygon();
  void init();
  bool alloc(int16_t aNumPoints);
  void dealloc();
  void dump();
  int32_t crc();
...
public:
  ...
  int16_t numPolygons;
  PolygonList();
  PolygonList(int16_t aNumPolygons);
  ~PolygonList();
  void init();
  bool alloc(int16_t aNumPolygons);
  void dealloc();
  void dump();
  int16_t numPoints();
  int32_t crc();
...
public:
  ...
  int16_t numNodes;
  NodeList();
  NodeList(int16_t aNumNodes);
  ~NodeList();
  void init();
  bool alloc(int16_t aNumNodes);
  void dealloc();
};
...
public:
  ...
  //int16_t targetPointIdx; // index of target point
  ...
  int16_t mowPointsIdx;    // next mowing point in mowing point polygon
  int16_t dockPointsIdx;   // next dock point in docking point polygon
  int16_t freePointsIdx;   // next free point in free point polygon
  int16_t percentCompleted;
  ...
  bool setPoint(int16_t idx, float x, float y);
  // set number points for waytype
  bool setWayCount(WayType type, int16_t count);
  // set number points for exclusion
  bool setExclusionLength(int16_t idx, int16_t len);
  void clearMap();
  void dump();
  bool load();
  bool save();
...
private:
  ...
  int16_t findNextNeighbor(NodeList &nodes, PolygonList &obstacles, Node &node, int16_t startIdx);
  ...
  int16_t linePolygonIntersectionCount(Point &src, Point &dst, Polygon &poly);
