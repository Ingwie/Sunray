...
uint32_t nextMQTTPublishTime = 0;
uint32_t nextMQTTLoopTime = 0;
...
void mqttReconnect()
{
  ...
  if (!mqttClient.connected())
    {
      wxLogMessage("MQTT: Attempting connection...");
      ...
      if (mqttClient.connect(clientId.c_str()))
        {
#endif
          wxLogMessage("MQTT: connected");
          // Once connected, publish an announcement...
          //mqttClient.publish("outTopic", "hello world");
          // ... and resubscribe
          wxLogMessage("MQTT: subscribing " MQTT_TOPIC_PREFIX "/command");
          mqttClient.subscribe(MQTT_TOPIC_PREFIX "/command");
        }
      else
        {
          wxLogMessage("MQTT: failed, rc= %i", mqttClient.state());
        }
    }
}


void mqttCallback(char* topic, int8_t* payload, uint16_t length)
{
  wxLogMessage("MQTT: Message arrived [" + (wxString)topic + "] ");
  cmd = "";
  for (int16_t i = 0; i < length; i++)
    {
      cmd += (char)payload[i];
    }
  wxLogMessage("%s", cmd.c_str());
      ...
      if (mqttClient.connected())
        {
          ...
          if (metronom == 16)
            {
              mqttRequestTopic |= MQTT_REQUEST_ONLINE;  // slowest
              metronom = 0;
            }
...
\"target\":{\"x\":%f,\"y\":%f},\"job\":%i,\"sensor\":%i,\"amps\":%.2f,\"map_crc\":%i,\"lateral_error\":%.2f,\
          ...
          if ((mqttRequestTopic & MQTT_REQUEST_STATS) == MQTT_REQUEST_STATS)
            {
              ...
              snprintf (mqttTxPayload, MQTT_MAX_PACKET_SIZE, \
                        "{\"duration_idle\":%u,\"duration_charge\":%u,\"duration_mow\":%u,\"duration_mow_float\":%u,\"duration_mow_fix\":%u,\
\"counter_float_recoveries\":%u,\"distance_mow_traveled\":%.1f,\"time_max_dpgs_age\":%.2f,\"counter_imu_triggered\":%u,\
\"temp_min\":%.1f,\"temp_max\":%.1f,\"counter_gps_chk_sum_errors\":%u,\"counter_dgps_chk_sum_errors\":%u,\"time_max_cycle\":%.3f,\
\"serial_buffer_size\":%u,\"duration_mow_invalid\":%u,\"counter_invalid_recoveries\":%u,\"counter_obstacles\":%u,\"free_memory\":%i,\
\"reset_cause\":%u,\"counter_gps_jumps\":%u,\"counter_sonar_triggered\":%u,\"counter_bumper_triggered\":%u,\
\"counter_gps_motion_timeout\":%u,\"duration_mow_motor_recovery\":%u}", \
          ...
          //wxLogMessage("MQTT build and send duration: ");
