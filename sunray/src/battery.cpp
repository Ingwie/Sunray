...
void Battery::enableCharging(bool flag)
{
  ...
  wxLogMessage("enableCharging %b", flag);
...
{
  wxLogMessage("battery.setIsDocked %b", state);
  docked = state;
}
...
{
  wxLogMessage("switching-off battery by operator...");
      ...
      if (chargingVoltage > 7)
        {
          ...
          wxLogMessage("CHARGER CONNECTED chgV=%f batV=%f", chargingVoltage, batteryVoltage);
          ...
          if (chargingVoltage <= 5)
            {
              ...
              wxLogMessage("CHARGER DISCONNECTED chgV=%f batV=%f", chargingVoltage, batteryVoltage);
            }
        }
      timeMinutes = (millis()-chargingStartTime) / 1000 /60;
      if (underVoltage())
        {
          wxLogMessage("SWITCHING OFF (undervoltage) batV=%f < %f", batteryVoltage, batSwitchOffIfBelow);
          buzzer.sound(SND_OVERCURRENT, true);
          if (switchOffAllowedUndervoltage)  batteryDriver.keepPowerOn(false);
        }
      else if ((millis() >= switchOffTime) || (switchOffByOperator))
        {
          wxLogMessage("SWITCHING OFF (idle timeout)");
          buzzer.sound(SND_OVERCURRENT, true);
                  ...
                  if (chargingVoltBatteryVoltDiff < -3.0)   // allow larger resistance (as mower chassis is getting older)
                    {
                      ...
                      wxLogMessage("CHARGER BAD CONTACT chgV=%f batV=%f diffV=%f slope(v/min)=%f", chargingVoltage, batteryVoltage, chargingVoltBatteryVoltDiff, batteryVoltageSlope);
                    }
                }
            }
