...
uint32_t nextWifiClientCheckTime = 0;

// use a ring buffer to increase speed and reduce memory allocation
ERingBuffer buf(8);
int16_t reqCount = 0;                // number of requests received
uint32_t stopClientTime = 0;
  ...
  if (!wifiClient.connected() || (wifiClient.available() == 0))
    {
      if (millis() > nextWifiClientCheckTime)
        {
          wifiClient.stop();
          wxLogMessage("WIF: connecting..." RELAY_HOST);
          if (!wifiClient.connect(RELAY_HOST, RELAY_PORT))
            {
              wxLogMessage("WIF: connection failed");
              nextWifiClientCheckTime = millis() + 10000;
              return;
            }
          wxLogMessage("WIF: connected!");
...
void processWifiAppServer()
{
      ...
      if (stopClientTime != 0)
        {
          if (millis() > stopClientTime)
            {
#ifdef VERBOSE
              wxLogMessage("app stopping client");
#endif
              client.stop();
              stopClientTime = 0;
            }
  ...
  if (!client)
    {
      //wxLogMessage("client is NULL");
      client = server.available();
    }
  if (client)                                 // if you get a client,
    {
#ifdef VERBOSE
      wxLogMessage("New client");             // print a message out the serial port
#endif
      battery.resetIdle();
      buf.init();                               // initialize the circular buffer
