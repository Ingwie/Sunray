...
// reads a uint8_t into result[*index] and increments index
                  ...
                  if (callback)
                    {
                      ...
                      memmove(this->buffer+llen+2,this->buffer+llen+3,tl); /* move topic inside buffer 1 uint8_t to front */
...
bool PubSubClient::publish(const char* topic, const uint8_t* payload, uint16_t plength)
{
...
int16_t PubSubClient::endPublish()
{
  return 1;
}
...
size_t PubSubClient::buildHeader(uint8_t header, uint8_t* buf, uint16_t length)
{
  ...
  buf[4-llen] = header;
  for (int16_t i=0; i<llen; i++)
    {
      buf[MQTT_MAX_HEADER_SIZE-llen+i] = lenBuf[i];
    }
  return llen+1; // Full header size is variable length bit plus the 1-uint8_t fixed header
}
